...
if (!class_exists('Email_Validation_Dilli')) {
    class Email_Validation_Dilli
    {
        ...
        public function validate_email($emailID)
        {
            ...
            $response = wp_remote_request("https://deva.dillilabs.com/api/" . $this->options['dilli_pubkey_api'] . "/email/" . urlencode($emailID) . "?pagenow=" . urlencode($pagenow) . "&url=" . add_query_arg($wp->query_vars, home_url($wp->request)), $args);
