...
== Changelog ==
= 4.55 =
*  still more code tweaks for php 8
*  reverse the edit user link back to what it was for years.
*  added other edit user links using the wp functions get_edit_user_link and get_edit_profile_url

= 4.54 =
*  more code tweaks for php 8
*  added experimental options to display related users.  IE users whose id's are stored in the main users meta.  This could be multiple meta records or an array of userids.  Define the field type as 'user_ids'.  Latest version of amr-users will 
*  fix for amr-groupings where in some instances grouping values were being seemingly 'lost'.

= 4.53 =
*  fix default value typo for "show_xls"
*  code tweaks to try detect old rows_per_page scalar values in main settings and force variable to be array to avoid php warning.
*  tweaks to export list options to ensure new settings exported
*  tested on php 8.0.3

= 4.52 =
*  added option for amr-users-plus to format ID as a dashicon person (with no link).  Useful if want to include ID for searching like with a member number, but not display it.  Can optionally add linktype.
*  added option amr-users-plus to format free text & validated value as a labeled field, labelled with the field's nice name.  Single Field columns only.   Thus avoid using 'before' and 'after' and automates the labelling.  The field 'nicename' used can be updated.

= 4.51 =
*  temporarily lost the option to show a refresh link. We won't need this soon, however it is added back in now.
*  added option to show a total of the records returned at the top of the list.  Yes it's there in pagination but only IF there are enough records to make pagination show & there was a request and code was already there.  The 'record/records' text is in a span so that you can hide or replace the text using css.  It is also translateable.
*  standardise rows per page in DB.  We had already removed the overall default rows per page, unnecessary extra setting.  Now set per list only.
*  tweaked code so that if you have multiple meta records for one user and one metakey, it will treat it same way as an array of values. Previously was giving a debug message.
*  changed the admin 'list level settings' link to go straight to the 'Overview' > 'Settings' tab as that was what felt like needed to be.  Overview tab is moreb for tweaking the admin user list menus.
*  NB always refresh your browser cache (&check if have server cache etc) if things look odd.  Control F5 in firefox ;)

= 4.50 =
*  reversed some css file changes for now.  

= 4.49 =
*  changed 'public' setting to 'in_public' as 'public' is a reserved word.  Please check all & only public lists were upgraded correctly.
*  added ability to have age in years months
*  added ability to format the user id as a link to the next list, passing userid as parameter
*  added reminder to enter license keys for addons to be informed if updates are available
*  added dashicons font to frontend for using icon formats in userlists on front
*  brought foward code to sort, renumber lists if using amr-users-plus
*  brought foward code to specify short names of lists for use in menu's and to control which lists show in the admin menu
*  removed overall default rows per page to simplify - specify per list only now.
*  tweaked default css ever so slightly so that a non table userlist type numbered 3, will be single column centered, as example. Added a default minimum height for vcard in list2.
*  tested on 5.7

= 4.47 =
*  If amr-users stylesheet used, make dashicons available on the front end in case the theme in use does not.
*  Make a 'year' format available for amr-users-plus users
*  Add prompt when on amr-* admin pages or plugin page, to enter license keys to get plugin updates as some sites had not entered their keys and were thus not aware that there were updates.

= 4.46 =
*  Fixed bug where front end csv link option only showed if the plus addon was active or maybe not at all for some. 
*  For amr-users-plus-um users.  Handle the various ultimate member images stored under the uploads folder.
*  Create some basic default formatting routines for various ultimate member fields, like telephone, emails etc
*  Set field types in 'nice names' (reset or run find fields) so that any formats available via other addons like amr-users-plus will become available.  EG: image sizes become available.
*  Note: if amr-users-plus-social is active then many social media fields will be styled.

= 4.45 =
*  For amr-users-plus-s2member users who recently updated. This allows the previous fallback function to still fire properlyIF another format function has not changed the values already.  Needed espevcially for multi value fields to bring the S2 lables in.  Update to 3.2 of amr-users-plus-s2member also needed.

= 4.44 =
*  For amr-users-plus-s2member users who recently updated AND are NOT also using amr-users-plus AND want to list timestamps.  The fallback function if you didnt have format functions wasn't quite there if you didnt have plus, sadly.  Sorry.  There is now a fallback.

= 4.43 =
*  Moved the check for empty field value so will still call formatting routine.  Some fields are allowed to be empty and still get formatted.  EG: avatars are put together from the users email, not from an actual field value.

= 4.42 =
*  New date formatting routines were (for some datefields) treating blanks as the current date.  This is not right.  Empty date fields now will not call the date formatting routines.

= 4.41 =
*  A little bug snuck into the cache logging which meant you only saw the last entry.  Fixed thanks to @muratgu for flagging & debugging.

= 4.40 =
*  Enabled additional telephone number format for amr-users-plus.

= 4.39 =
*  Minor fix so that unserialize will cope with exported list settings that have html code in the before/after fields.  I'd rather you didnt use html there, there are lots of css classes so you should be able to target what you need without adding html, and risking invalid html.

= 4.38 =
*  Handle empty lists without notices
*  Tweak to improve the random shuffle of the user members.  Use shuffle=1 in the shortcode to shuffle the list, or show_randompage=1 to just start on a random page and keep the defined order.

= 4.37 =
*  Further improvements for formatting images, avatars, buddypress images, profile grid etc in various sizes. amr-user-plus update required.
*  Also enabling link to posts or custom post types if the id's are stored as user-meta.  amr-user-plus update required.

= 4.36 =
*  Added more field types for post_ids for uploaded files and images etc
*  Added serialised field type to cope with serialised arrays (sigh)
*  Removed a deprecated use of pub_priv_sql_capability
*  Removed unnecessary calls to helper functions that no longer exist (thanks Randy!)

= 4.35 =
*  Add a lot of extra field types for the addon plugins, eg: booleans, images, uploads_url, image, html_clour
*  Added settings for formats for some field types (eg: urls as icon (download), filename, image etc).  May require add-ons for the actual formatting.
*  Made the 'format' configure page sort the same as the field selection page
*  Tested on wp 5.5

= 4.34 =
*  Add field types for ratings & percent

= 4.33 =
*  Add upgrade check to add in default excel text link to main settings if missing 
*  Add option save if certain texts were blank & defaults added (like excel text) in settings screen.

= 4.32 =
*  Dummy update for missing  backend file & clarity of version

= 4.31 =
*  Added ability to use a date field that stores date NOT in ISO standard and NOT as Unix timestamp, and reformat it.   Specify the fieldtype  as 'ambiguous'. This will also handle dates stored as YYYY/MM/DD. Functions are then available in amr-users-plus to format dates as per ISO Standard dates. See https://www.php.net/manual/en/datetime.formats.php where Date values separated by slash are assumed to be in American order: m/d/y and Date values separated by dash are assumed to be in European order: d-m-y.  Dates that are too ambiguous (eg 23/12/2000 or 12-23-2000 will simply be echoed).   Note that dates stored without the year first, cannot be sorted accurately.

= 4.30 =
*   format timestamp as days ago must use GMT as timestamp is UTC.
*   add data-label to table html to make for easier 'css only responsive tables'
*   add css selector at list level so that individual lists can be targeted with custom css

= 4.29 =
*   made pluggable functions load before csv formatting function call as this was not right for s2 member EOT csv formatting.

= 4.28 =
*   fixed bug introduced in last version where CSV_filter did not work unless 'show csv' was ticked for the list. This highlighted that it was confusing for admin user lists to ignore list settings (list settings were for front end only). Now user lists in admin will adhere to list settings EXCEPT for the html type - because css is different front & back, and one may need to work with list in table fashion in back end.
*   tested on 5.3.2-alpha

= 4.27 =
*   Added code to faciliate excel list exports
*   Remove unused code that had a deprecated escape call  20190924
*   Ensure all sql calls use 'prepare'
*   tested on wp 5.3.1 alpha
