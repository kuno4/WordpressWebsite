...
Description: Configurable users listings by meta keys and values, comment count and post count. Includes  display, inclusion, exclusion, sorting configuration and an option to export to CSV. If you found this useful, please <a href="http://wordpress.org/extend/plugins/amr-users/">rate it</a>, or share it.
Author: anmari
Version: 4.55
...
if (is_admin()) {
	...
	include('includes/ameta-admin-prefix-check.php');
}
...
function add_ameta_stylesheet () {
      ...
      if ( file_exists($myStyleFile) ) { 
            wp_register_style('auserslist', $myStyleUrl); //20191227 to avoid class with social
            wp_enqueue_style( 'auserslist', $myStyleUrl);
      }
...
global $amain;
	...
	if (is_user_logged_in()
		...
		if (empty($amain)) $amain = ausers_get_option('amr-users-main');	
		if (!empty($amain['is_public'][$list])) return true; //20210314
		else return false;
	}
	return false;
...
function add_amr_stylesheet() { // for front
global $amain;
	if (empty($amain)) $amain = ausers_get_option('amr-users-main');
    ...
    if ( file_exists($myStyleFile) ) {
     ...
     wp_enqueue_style('dashicons', 999); //20210228  
   }
...
function amr_meta_load_js() { //20210311
	...
	if  ((!function_exists ('is_admin')) /* eg maybe bbpress*/ or (is_admin())) {
		...
		//add_filter('contextual_help',		'amrmeta_mainhelp',10,3);		 // contextual help deprecated

		if (isset($_GET['page']) && ($_GET['page'] == 'ameta-admin-overview.php')) { // only add js on plugin organise page 
			add_action ('admin_enqueue_scripts', 	'amr_meta_load_js'); //20210311
		}
	}
	...
	add_action ('wp_loaded',				'ausers_load_pluggables', 1);  // was after wp_loaded which would be after theme 
	add_action ('init',						'ausers_add_actions', 99);		
	add_action ('wp_print_styles', 			'add_amr_stylesheet');
	add_action( 'admin_enqueue_scripts',	'add_amr_stylesheet');
	...
	add_action ('wp_loaded',				'amr_meta_handle_export');
	add_action ('wp_loaded',				'amr_meta_handle_csv');
