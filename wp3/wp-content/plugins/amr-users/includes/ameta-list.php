...
function amr_rows_per_page($rpp=20){  //check if rows_per_page were requested or changed, set default if nothing passed
...
function amr_first_showing() { // use GET not REQUEST to be sure it is dynamic input ? 
	if ((!isset($_REQUEST['filter'])) and
		...
		(!isset($_REQUEST['sort'])) 
		and
		(!isset($_GET['filter'])) and
		(!isset($_GET['su'])) and
		(!isset($_GET['clear_filtering'])) and
		(!isset($_GET['listpage'])) and
		(!isset($_GET['rows_per_page'])) and
		(!isset($_GET['refresh'] ))and
		(!isset($_GET['dir'])) and
		(!isset($_GET['sort'])) 
	)
...
function amr_convert_indices ($lineitems, $icols) {
		...
		//debug_this($lineitems, 'lines items');
		$line = array();
		$icols = array_unique($icols);  // because we may have the grouping field tagged on the end
		foreach ($icols as $ic => $c) { 
			...
			if (isset($lineitems[$ic])) {
				//echo '<br />we got $lineitems[$ic] '.$ic.' '.$lineitems[$ic];
				$w = $lineitems[$ic];
			}
			else { 
				$w = '';
			}
			//echo '<br />   we got w'.$w. '    c='.$c;
			$line[$c] = stripslashes($w);
			//echo $line[$c];
		}
		//debug_this($line, ' line');
		return ($line);
...
function alist_one_widget ($type, $i=1, $do_headings=false, $do_csv=false, $max=10){ // 20210414 for php 8
...
//		else $cols = str_getcsv( $line[1]['csvcontent'], ',','"','\\');
		...
		foreach ($lines as $il => $lineitems) {
			...
			foreach ($icols as $ic => $c) { /* use the icols as our controlling array, so that we have the internal field names */
				...
				$v = $lineitems[$ic];
				$linehtml .= '<td class="'.$c.'">'.amr_format_user_cell($c, $v, $user, $l). '</td>';
			}
...
function alist_one($type, $ulist ,$options) { //20210414 for php 8
...
global $amr_refreshed_heading, $totalitems; 	
	...
	if (empty ($ulist) or empty($aopt)) {  // use first list?
		$ulist = key($aopt['list']);
	}
/*	else {
...
/**** later	if (!empty($amain['names'][$ulist]))
		$caption = $amain['names'][$ulist];
	else */	
		$caption 	= '';
	$sortedbynow = '';
	
	
	//debug_this($amain, ' amain for rows per page');
	
	if (empty($amain['rows_per_page'][$ulist])) {   // 20210407 explicitly force 'rows_per_page' to be an array 
		$amain['rows_per_page'] = array($ulist => 20);
		//$amain['rows_per_page'][$ulist]=20;
	}
		
	$rowsperpage = amr_rows_per_page($amain['rows_per_page'][$ulist]); // will check request
...
// figure out what we are doing - searching, filtering -------------------------------------------------------
	...
	if ($totalitems < 1) {
			$html = __('No lines found.','amr-users').amr_users_get_refresh_link($ulist);  // return don't echo 20191225
			return $html;
	}
...
//------------------------------------------------------------------------------	 check for sort 
	...
	// do csv filter here ?
		...
		if (!empty($_REQUEST['csvsubset']) and (!empty($amain['show_csv'][$ulist]) or !empty($amain['show_xls'][$ulist] ))) {
			// to avoid redoing if widget on same page
			$csvlines = amr_csvlines_to_csvbatch($lines, $ulist);
			$html = amr_lines_to_csv($csvlines, $ulist, true, false,'csv','"',',',chr(13).chr(10) ); //*** need form?
			return $html;
		}
		else {	
