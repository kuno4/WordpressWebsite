	...
	'<ul class="subsubsub" style="float:left; white-space:normal;">';
		...
		if (current_user_can('list_users') or current_user_can('edit_users')) {
			echo '<li style="display:block; float:left;">'
				.au_csv_link($t, $l.'&amp;suffix=csv', $n.__(' - Standard CSV.','amr-users')).'</li>';
			echo '<li style="display:block; float:left;"> |'.au_csv_link(__('Txt Export','amr-users'),
						$l.'&amp;csvfiltered&amp;suffix=txt',
		...
		echo '<li style="display:block; float:left;"> | '
			...
			.'</li>';
		echo  '<li style="display:block; float:left;"> | '.au_lists_link(__('View','amr-users'), $l,$amain['names'][$l],'userlists').'</li>';	//20210312
...
//	.PHP_EOL.'<div style="clear:both;"><br /></div>'; //20191226 remove for more flexible styling
//	$text .= '</form>';
	return ($text);
}
...
global $amain;
	...
	$rowsperpage = amr_rows_per_page($amain['rows_per_page'][$i]);  // will check for request
