...
function amr_delete_user_meta($delete) {
	...
	$string = "'".implode("','",$text)."'"; //202010315  swop the array keys due to deprecation
...
function amr_confirm_delete_user_meta($del) {
	...
	foreach ($del as $name=> $on) {
		echo PHP_EOL.'<br /><input type="checkbox" id="delete'.$name.'"  name="delete['.$name.']"';
		echo ' value=true checked="checked" ';
		echo ' /> '.$name;
	}
	echo PHP_EOL.'<p><input class="button-primary" type="submit" name="deleteconfirmed" value="'. __('Confirm Delete', 'amr-users') .'" /></p>';
...
function amr_meta_keys_page() {
	...
	elseif (isset($_POST['delete'])) {// and ($_POST['delete'] === "Delete")) {
		...
		echo ausers_form_end();  // 20191215 form was unclosed and thus broken by profile grid
		return;
	}
