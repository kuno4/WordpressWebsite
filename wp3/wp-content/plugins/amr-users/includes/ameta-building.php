...
// now need to make sure we find all the meta keys we need
	...
	if ( ! is_admin() ) {
		...
		if (!($locale === 'en_US')) { 
			...
			amr_need_the_field($amr_current_list, 'roles')) {	//*** should actually be 'prefix-capabilities'
...
// we just need to expand the meta data
		if (!empty($keys)) { // - the list of metadata keys.  If we have some meta data requested, and most of the time we will
			foreach ($keys as $i2 => $v2) {	
				...
				//debug_this($test, ' get missing mta');
				...
				if (!empty($test)) { 
					...
					//$test = apply_filters('amr_pre_process_raw_data', $test, $i2);  //2020
					if (is_array($test)) {  // because we are now checking for multiple values so it returns an array
						...
						// otherwise they really should create their meta data a better way. Can't solve everyones problems.
							//debug_this($test, ' In Building Cache, multiple meta found for '.$i2.' for user'.$userobj->ID.' '); //20210225
					...
					if (!empty($userobj->$i2)) { // 20210317 cope with multiple meta records - will be a csv string
						$users[$userobj->ID][$i2] = $userobj->$i2; 
					}
...
//	track_progress('Getting data for network='.$network);
	...
	//add_filter('pub_priv_sql_capability', 'amr_allow_count');// checked by the get_posts_by_author_sql // deprecated 202009
...
//								$line[99999] = '';		
		...
		if (!$amrusers_fieldfiltering) { // if we are not just doing a real time filtering where we will not have full data then do cache stuff
			if (!isset($count)) $count = 0;  //20200928
		...
		if (!empty($lines)) {
			//debug_this($lines, 'at end return aybe cache');	
			return ($lines);
		}
		else return false;
}
