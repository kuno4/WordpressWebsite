...
function which_list() { //because we may try to have lists as submenu items and wp doesnt like parameters in the slug
	global $amain, $amr_current_list;

//		debug_this($_REQUEST, ' whats in request');
		$list_number = false;
		if (!empty($_REQUEST['list'])) {
			$list_number = (int) $_REQUEST['list'];
			//unset ($_REQUEST['list']); // donlt unset for some reason we call this twice 
		}
		elseif (!empty($_REQUEST['page']) and stristr($_REQUEST['page'],  'list=')) {  // because wp admin doesnt like to let us pass parameters in submenus
			$list_number = (int) str_ireplace('userlist?list=','',$_REQUEST['page']);
		}
		else {
			$list_number = array_key_first($amain['names']);
		}
		$amr_current_list = $list_number;
		return ($list_number);	
}

function debug_this($what, $text='' ) {
	IF (WP_DEBUG) echo '<pre>'.$text.' '.print_r($what, true).'</pre>';   //only if actually debugging 
}
	
function amr_get_local_url_if_file_exists($partialurl, $args) { // used in amr-users-plus
	// check uploads folder (with possible filter for plugins like ultimate member and buddypress x profile) 
	// for file existence
	// if image and size requested, check for matching file size
	
		$upload_dir   = wp_upload_dir();		
		$localdir = apply_filters('amr_wp_uploads_dir',$upload_dir['basedir'], $args);
		$localfile = $localdir.DIRECTORY_SEPARATOR.$partialurl;
		$sizename = ''; // updated only for images
		if (!file_exists($localfile)) { // if the original file dpes not exist
				return false;	
		}
		$url = array();
		//debug_this($args,' args');
		if (isset ($args['size'])) {// then we doing an image?		
			list($width, $height, $type, $attr) = getimagesize($localfile); 
			$wh = amr_users_adjust_image_sizes(array('width'=>$width, 'height'=>$height), $args['size']); // get the wordpress image sizes
			$sizename = '-'.$wh['width'].'x'.$wh['height'];
			$url['w'] = $wh['width'];
			$url['h'] = $wh['height'];
			//debug_this($sizename, ' size name ');
			
			$ext = pathinfo($localfile, PATHINFO_EXTENSION);
			$localsizefile = str_replace('.'.$ext, $sizename.'.'.$ext, $localfile);
			//if sie exisst

		//debug_this($localsizefile, ' local size file ');
			if (!file_exists($localsizefile) ) 
				$sizename = ''; // just use original
		}
		
		// ultimate member uses the filter
		$localurl = apply_filters('amr_wp_uploads_url',$upload_dir['baseurl'],$args);
		$localurl = $localurl.DIRECTORY_SEPARATOR.$partialurl;
		//debug_this($localurl, 'before size replace');
		if (!empty($sizename)) //will be emoty if not an image file, or smaller file does not exist.
			$localurl = str_replace('.'.$ext, $sizename.'.'.$ext, $localurl);
		
		$url['localurl'] = $localurl;
		//debug_this($url, ' the local url');
		return $url; 
}

function amr_get_nice_name($fld) {
	global $amr_nicenames;
	if (empty($amr_nicenames)) 
		$amr_nicenames = get_option('amr-users-nicenames');	
	if (!isset($amr_nicenames[$fld])) return $fld;	
	else return $amr_nicenames[$fld];
}

function amr_get_file_name_from_url($url) {
	if (empty($url)) return '';
	$array=explode("/",$url);
	$filename = end($array);
	return ($filename);		
...
function amru_new_DateTime_nonfatal ($datetimestring, $tzobj) { // date time create with exception trap to avoid fatal errors
		...
		try {	
			...
			if ((current_user_can('manage_options') or current_user_can('manage_user_lists')) and (!empty($check['warning_count']))) {
				echo '<p>Warning: php has done its best with the data, but it is not entirely happy:</p>';
				print_r($check, true);
			}
			$done = $dt->setTimezone($tzobj);		
			if (!$done) echo '<p>setTimezone failed for '.print_r($dt, true).'</p>';
			}
		catch(Exception $e) {
			if (current_user_can('manage_options') or current_user_can('manage_user_lists')) {
			echo '<p>'.$e->getMessage().'</p>';  // 2017 08 18 get actual message
			echo '<p>'
			.__('Message to admin only:','amr-users').' '
			.__('Fatal error caught','amr-users').' - '.
			sprintf(__('Unable to create DateTime object from this string: "%s" ','amr-users'),$datetimestring).' with timezone '.print_r($tzobj, true)
...
function amru_get_func_name ($fld, $format) {
		...
		if (empty($amr_users_field_types)) // to get the type of field
			$amr_users_field_types = amrmeta_get_fieldtypes(); //20100119	
		...
		$funcname = 'ausers_format_'.$type.'_'.$format;
		return $funcname;
}
...
global $amain, $amr_current_list;
	...
	if (!empty($_REQUEST['rows_per_page'])) {
		...
		if (!($rpp === $amain['rows_per_page'][$amr_current_list]))  //20210407 if not remembering rows per page well
...
function amr_users_can_edit ($type) {
		if (is_admin() and (current_user_can('manage_options') or current_user_can('manage_user_lists') )
...
global $amr_current_list;
	//if (empty($amr_current_list)) $amr_current_list = key($amain['names']); //202001 in case we deleted first list
	...
	if (!empty($_REQUEST['reqxls'])) 
		$amr_current_list = intval ($_REQUEST['reqxls']);
	elseif (!empty($_REQUEST['reqcsv'])) 
		$amr_current_list = intval ($_REQUEST['reqcsv']);
	elseif (!empty($_REQUEST['ulist']))	
		$amr_current_list = intval ($_REQUEST['ulist']);
		
	if (!empty($_REQUEST['rows_per_page'])) { 
		if (!($_REQUEST['rows_per_page'] == $amain['rows_per_page'][$amr_current_list]) )
			$base = add_query_arg('rows_per_page',(int) $_REQUEST['rows_per_page'],$base);
	}
	 
	$html = PHP_EOL.'<div class="wrap"><!-- form wrap -->'.PHP_EOL;
	$html .= PHP_EOL.'<form id="userlist" class="userlist'.$amr_current_list.'" action="'.$base.'" method="post">'; //20191226 add userlist class to form so can style separately
	$html .= PHP_EOL.'<input type="hidden" name="action" value="save" />';
	$html .= PHP_EOL.'<input type="hidden" name="ulist" value="'.$amr_current_list.'" />';  //$amr *** not always set?
