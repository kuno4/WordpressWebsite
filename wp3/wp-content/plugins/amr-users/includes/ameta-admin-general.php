...
function amrmeta_validate_mainoptions()	{ 
	...
	if (isset($_POST['xls_text'])) { 
		$return = amrmeta_validate_text('xls_text');
		if ( is_wp_error($return) )	echo $return->get_error_message();
	}
...
function amr_meta_general_page_display() { 
	global $amain;
		...
		ausers_update_option('amr-users-main', $amain);  // save so user doesnt have to
...
//			.$disabled			
		...
		echo '<br /><br />'.PHP_EOL.
		'<label for="xls_text">';
		_e('Text for xls link', 'amr-users'); 
		echo ' <em>';
		_e('(Requires amr-users-plus)', 'amr-users').' '._e('Requires PHPOffice/Spreadsheet');
		echo ' </em>';
		echo '</label><br />'.PHP_EOL.
		'<input type="text" size="130" id="xls_text" 
					name="xls_text" value="';
		echo esc_attr($amain['xls_text']); 
		echo '"/>'.' '.__('Preview:','amr-users').' '.
		'<a href="#" title="'.__('This will be a link','amr-users').'" >'. $amain['xls_text'].'</a>';
...
/*		echo '<br /><br />
		...
		echo '"/>';
		*/
/*	20210407 remove general avatar size & use sizes per list only
echo '<br /><br />
