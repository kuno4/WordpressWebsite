...
/* ---------------------------------------------------------------------- */	
	...
	function log_cache_event($text) {
		...
		$sql = $wpdb->prepare($sql,
			date_format($datetime,'Y-m-d H:i:s') ,$text);
	...
	function get_cache_report_lines ($reportid, $start,  $rowsperpage, $shuffle=false ) { /* we don't want the internal names in line 0, we just want the headings and the data from line 1 onwards*/
		global $wpdb,$totalitems;	
		$wpdb->show_errors();	
		
		if ($shuffle) {
			$orderby = ' ORDER BY RAND() '; //20200928 make truly random
			}
		else	
	...
	function search_cache_report_lines ($reportid, $start,  $rowsperpage, $searchtext, $shuffle=false ) { /* we don't want the internal names in line 0, we just want the headings and the data from line 1 onwards*/
			...
			if ( is_wp_error($results) )	{	
				...
				if (!empty($summary)) { 	
					...
					foreach ($summary as $rd => $rpt) {
						...
						echo '<tr>'
						.'<td>'.$rpt['rid'].'</td>'
						.'<td>'.au_lists_link(__('View','amr-users'),$rd,$rpt['name'],'userlists').'</td>' //20210314
