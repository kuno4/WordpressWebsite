...
class CMEB_EmailBlacklist
{
	...
	{
		isset($_POST['cmeb_email_black']) ? $var = $_POST['cmeb_email_black'] : $var = $_GET['cmeb_email_black'];

		switch ($var)
		{
			case 'add':
				self::addEmail($_POST['black_email']);
				break;
			case 'edit':
				$ids = $_POST['black_id'];
				$emails = $_POST['black_email'];
				foreach ($ids as $key => $id) {
					self::editEmail($id, $emails[$key]);
				}
				break;
			case 'delete':
				self::deleteEmail($_GET['black_id']);
				break;
		}
